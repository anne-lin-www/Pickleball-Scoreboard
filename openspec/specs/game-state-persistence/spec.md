# game-state-persistence Specification

## Purpose

Defines how the pickleball scoreboard persists active game state across page reloads and browser sessions, enabling players to resume a game after an accidental refresh (F5 silent restore) or after closing and reopening the tab (ResumeDialog). Covers localStorage save/restore, sessionStorage session flag, the ResumeDialog component, game class serialization interface, the `useGamePersistence` hook, and explicit-exit cleanup.
