# ui-theme-switch Specification

## Purpose

The theme switch feature enables the operator to toggle between two custom DaisyUI themes: `court-day` (light) and `midnight-pro` (dark), both built on the Indigo Pro palette. The selected theme is applied to the `data-theme` attribute on the document root and persisted across sessions via localStorage.
